...
*   **Context/Environment:** Jekyll Static Site Generator, GitHub Pages deployment, `_posts` directory management.
*   **Root Cause:** The Jekyll core engine is hardcoded to recognize the `_posts` directory as a chronological collection; it requires a strict `YYYY-MM-DD-title.md` format to parse the file as a "Post" object and populate the `site.posts` array.
